...
import ua.profarma.medbook.utils.Constants;
        ...
        switch (requestCode) {
            case PERMISSIONS_REQUEST_LOCATION: {
                ...
                if (grantResults.length > 0
                    ...
                    LogUtils.logE(getClass().getCanonicalName(), "Location permission Granted");
                    // permission was granted, yay! Do the contacts-related task you need to do.
                    showFragment(NewMedicalInstitutionFragment.newInstance(), TAG_MAP, false);
                } else {
                    LogUtils.logE(getClass().getCanonicalName(), "Location permission Denied");
            ...
            case Constants.READ_PHONE_STATE_PERMISSION_REQUEST_CODE:
                if (grantResults.length > 0
                        && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    LogUtils.logE(getClass().getCanonicalName(), "READ_PHONE_STATE permission Granted");
                    // permission was granted, yay! Do the contacts-related task you need to do.
                    readPhoneInfo();
                } else {
                    LogUtils.logE(getClass().getCanonicalName(), "READ_PHONE_STATE permission Denied");
                    finishAffinity();
                    // permission denied, boo! Disable the functionality that depends on this permission.
                }
                break;
